    ...
    async def 경민아(self, ctx, *args):
        ...
        try:
            ...
            if len(args)==3 and (args[2]=='풀었어'):
                if args[0].isdigit():
                    problem, boj_handle = args[0], args[1]
                else:    
                    problem, boj_handle = args[1], args[0]
                if boj.is_solved(args[0], args[1]):
                    await ctx.reply('진짜넹')
                else:
        ...
        try:
            ...
            if messages[0].content==today and self.problems:
                ...
                for user, problem in self.problems:
                    if boj.is_solved(problem[1], boj_id[user]):
